...
Palantir is a local-only desktop app (Tauri 2 + Rust backend, React/TypeScript frontend) for managing Kanban tickets executed by local code agents — Codex, Claude, and OpenCode are wired in and selectable per ticket. Each ticket runs on a dedicated Git branch and lands in a Review column — the app never merges, pushes, deletes branches, or configures Git credentials itself.
